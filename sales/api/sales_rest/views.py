...
def api_customer_list(request):
    if request.method == "GET":
        ...
        try:
            ...
            return JsonResponse(
                {"customers": customer}, encoder=CustomerEncoder, safe=False
            )
        except:
            response = JsonResponse({"message": "Invalid Customer Info"})
...
def api_salesperson_list(request):
    if request.method == "GET":
        ...
        try:
            ...
            response = JsonResponse({"message": "Invalid Salesperson Info"})
            response.status_code = 400
            return response


@require_http_methods(["GET"])
def api_automobile_list(
    request,
):
    if request.method == "GET":
...
def api_sales_record_list(request, automobile_vo_id=None):
    if request.method == "GET":
        if automobile_vo_id is not None:
            ...
            return JsonResponse(salesrecords, encoder=SalesRecordEncoder, safe=False)
        else:
            salesrecords = SalesRecord.objects.all()
        return JsonResponse(
            {"salesrecords": salesrecords}, encoder=SalesRecordEncoder, safe=False
        )
    else:
        automobile = None
